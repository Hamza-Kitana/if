...
class Title {
  ...
  createMesh() {
    ...
    // place text inside the image near bottom
...
class Media {
  ...
  createShader() {
    const texture = new Texture(this.gl as any, {
      ...
      minFilter: (this.gl as any).LINEAR,
      magFilter: (this.gl as any).LINEAR,
      flipY: true
    } as any);
    this.program = new Program(this.gl as any, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Discard outside rounded box to avoid blending artifacts
          if (d > 0.0) discard;
          vec3 baseColor = color.rgb;
          // Bottom white gradient to improve legibility of bottom text
          float whiteGrad = 1.0 - smoothstep(0.0, 0.65, vUv.y);
          baseColor = mix(baseColor, vec3(1.0), whiteGrad * 0.45);
...
class OglApp {
  ...
  constructor(
    ...
    this.scroll = { ease: scrollEase, current: 0, target: 0, last: 0 } as any;
  ...
  createRenderer() {
    ...
    this.container.appendChild((this.gl as any).canvas);
  }
  createCamera() {
  ...
  createMedias(items: Array<GalleryItem>, bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems: Array<GalleryItem> = [
      ...
      { image: `https://picsum.photos/seed/3/800/600?grayscale`, text: 'Item 3' }
    ];
  ...
  handleSelect() {
    ...
    const width = (this.medias[0] as any).width;
    const idx = Math.round(Math.abs(this.scroll.target) / width) % this.items.length;
  ...
  update() {
    ...
    if (this.medias) {
      this.medias.forEach(media => media.update(this.scroll, direction as any));
    }
  ...
  addEventListeners() {
    ...
    // Scope interactions to the gallery container only
    ...
    this.container.addEventListener('mouseup', this.boundOnTouchUp as any);
    ...
    this.boundOnEnter = () => { this.isHovered = true; };
    this.boundOnLeave = () => { this.isHovered = false; };
    this.container.addEventListener('mouseenter', this.boundOnEnter as any);
    this.container.addEventListener('mouseleave', this.boundOnLeave as any);
  }
  boundOnResize!: () => void;
  ...
  destroy() {
    ...
    this.container.removeEventListener('mouseup', this.boundOnTouchUp as any);
    ...
    this.container.removeEventListener('mouseenter', this.boundOnEnter as any);
    this.container.removeEventListener('mouseleave', this.boundOnLeave as any);
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const app = new OglApp(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, autoScrollSpeed, onSelect });
    return () => {
      app.destroy();
    };
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, autoScrollSpeed, onSelect]);
